 ...
 *                                                                      *
    This source code is for the Teensy 4.0 board
 ...
 *                                                                      *
      DNMS - Digital Noise Measurement Sensor
      Copyright (C) 2021, 2022, 2023, 2024, 2025  Helmut Bitter
      Copyright (C) 2021, 2022  Burkhard Volkemer for FFT software parts
      Copyright (C) 2025  Burkhard Volkemer 
 *                                                                      *
...
#define SOFTWARE_VERSION "DNMS Version 5.5.x"  // Version 5.5.x for Teensy4 and ICS-43434 or IM72D128 microphone based on FFT
/**********************************************************************************
 *                                                                                *
...
#define BLINK_PERIOD_ICS43434 100  // Time  in ms for blinking of internal red LED for ICS-43434 microphone
#define BLINK_PERIOD_IM72D128 500  // Time  in ms for blinking of internal red LED for IM72d128 microphone
...
double *corr1024_Z;
double *corr1024_A;
uint16_t micro_type; // 1 = ICS-43434, 2 = IM72D128
...
#define window 0.3750  // Hanning
  ...
  dnms_decimation.begin(dnms_decimate_coeffs, number_FIR_taps_decimation, DECI_FACTOR, BLK_LEN, &decimation_buffer[0], &decimation_state_buffer[0]);
...
void loop() {
  ...
  if (start_phase) {                                  // start phase is as long as there is no DNMS Reset Command from I²C Master
  ...
  } else { /*** here starts the normal loop besides the start phase after the I²C Master has send a DNMS Reset Command  ***/
    // ***** FFT4096 available?
    if (dnms_fft.available()) {
      fft_all_Z = 0;
      fft_all_A = 0;
    ...
    /**** blink_period (100ms or 500ms depending on microphone) over? ***/
    ...
    // calculate LZeq and LAeq over number of FFTs and store/check min and max values
    if (mean_count_fft >= NUMBER_OF_FFTs) {
      ...
      switch (micro_type) {
        ...
        case 2:
          leq_fft_A = 133.0102999565 + 10 * log10(z);
          break;
      ...
      switch (micro_type) {
        ...
        case 2:
          leq_fft_Z = 133.0102999565 + 10 * log10(z);
          break;
      ...
      for (i = 14; i < 32; i++) {
        ...
        switch (micro_type) {
          ...
          case 2:
            leq_freq_band_Z[i] = 133.0102999565 + 10 * log10(z);
            break;
        ...
        switch (micro_type) {
          ...
          case 2:
            leq_freq_band_A[i] = 133.0102999565 + 10 * log10(z);
            break;
    ...
    // ***** ZoomFFT1024
    if (mean_count_fft1024 >= NUMBER_OF_FFTs) {
      ...
      for (i = 1; i < 14; i++) {
        ...
        switch (micro_type) {
          ...
          case 2:
            leq_freq_band1024_Z[i] = 133.0102999565 + 10 * log10(z);
            break;
        ...
        switch (micro_type) {
          ...
          case 2:
            leq_freq_band1024_A[i] = 133.0102999565 + 10 * log10(z);
            break;
    ...
    if (calculate_leq_2nd) {
      //      Serial.println("calculate_leq_2nd");
...
void DNMS_reset() {
  ...
  // Reset internal values for 2nd measurement
  measure_loop_count_2nd = 0;
  sum_leq_g_A_2nd = 0;
  leq_g_A_2nd = 0;
  leq_A_min_2nd = 0;
...
void i2c_receive_from_master(int num_bytes) {
  ...
  switch (command_received) {
    case 1:  // Reset DNMS
      // Reset internal values for 1st measurement
      AudioNoInterrupts();
      DNMS_reset();
      start_phase = false;    
      break;
