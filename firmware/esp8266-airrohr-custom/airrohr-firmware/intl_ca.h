/*
 *	airRohr firmware
 *	Copyright (C) 2016-2018 Code for Stuttgart a.o.
 *
 * Catalan translations
 *
 *	Texts should be as short as possible
 */

#define INTL_LANG "CA"
#define INTL_PM_SENSOR "Sensor de partícules fines"
const char INTL_CONFIGURATION[] PROGMEM = "Configuració";
#define INTL_WIFI_SETTINGS "Configuració WiFi"
#define INTL_WIFI_NETWORKS "Càrrega de xarxes wifi ..."
#define INTL_LANGUAGE "Idioma"
const char INTL_NO_NETWORKS[] PROGMEM = "Xarxa WiFi no detectada";
const char INTL_NETWORKS_FOUND[] PROGMEM = "Xarxes trobades: ";
const char INTL_AB_HIER_NUR_ANDERN[] PROGMEM = "Configuració avançada (només si saps el que fas)";
const char INTL_SAVE[] PROGMEM = "Desar";
const char INTL_SENSORS[] PROGMEM = "Sensors";
const char INTL_MORE_SENSORS[] PROGMEM = "Més sensors";
const char INTL_SDS011[] PROGMEM = "SDS011 ({pm})";
const char INTL_PMS[] PROGMEM = "Plantower PMS(1,3,5,6,7)003 ({pm})";
const char INTL_HPM[] PROGMEM = "Honeywell Sensor de partícules fines";
const char INTL_NPM[] PROGMEM = "Tera Sensor Next PM ({pm})";
const char INTL_NPM_FULLTIME[] PROGMEM = "Next PM fulltime";
const char INTL_SPS30[] PROGMEM = "Sensirion SPS30 ({pm})";
const char INTL_PPD42NS[] PROGMEM = "PPD42NS ({pm})";
const char INTL_DHT22[] PROGMEM = "DHT22 ({t}, {h})";
const char INTL_HTU21D[] PROGMEM = "HTU21D ({t}, {h})";
const char INTL_BMP180[] PROGMEM = "BMP180 ({t}, {p})";
const char INTL_BMX280[] PROGMEM = "BME280 ({t}, {h}, {p}), BMP280 ({t}, {p})";
const char INTL_SHT3X[] PROGMEM = "SHT3X ({t}, {h})";
const char INTL_SCD30[] PROGMEM = "SCD30 ({t}, {h}, CO₂)";
const char INTL_DS18B20[] PROGMEM = "DS18B20 ({t})";
const char INTL_DNMS[] PROGMEM = "DNMS ({l_a})";
const char INTL_DNMS_CORRECTION[] PROGMEM = "Correcció en dB (A)";
const char INTL_TEMP_CORRECTION[] PROGMEM = "Correcció en °C";
const char INTL_HEIGHT_ABOVE_SEALEVEL[] PROGMEM = "";
const char INTL_PRESSURE_AT_SEALEVEL[] PROGMEM = "";
const char INTL_NEO6M[] PROGMEM = "GPS (NEO 6M)";
const char INTL_BASICAUTH[] PROGMEM = "Autorització";
#define INTL_REPORT_ISSUE "Informar d'un problema"

const char INTL_FS_WIFI_DESCRIPTION[] PROGMEM = "Sensor WiFi en mode de configuració";
const char INTL_FS_WIFI_NAME[] PROGMEM = "Nom";
const char INTL_MORE_SETTINGS[] PROGMEM = "Més configuracions";
const char INTL_AUTO_UPDATE[] PROGMEM = "Actualització automàtica del Firmware";
const char INTL_USE_BETA[] PROGMEM = "Carregar firmware beta";
const char INTL_DISPLAY[] PROGMEM = "OLED SSD1306";
const char INTL_SH1106[] PROGMEM = "OLED SH1106";
const char INTL_FLIP_DISPLAY[] PROGMEM = "Pantalla OLED flip";
const char INTL_LCD1602_27[] PROGMEM = "LCD 1602 (I2C: 0x27)";
const char INTL_LCD1602_3F[] PROGMEM = "LCD 1602 (I2C: 0x3F)";
const char INTL_LCD2004_27[] PROGMEM = "LCD 2004 (I2C: 0x27)";
const char INTL_LCD2004_3F[] PROGMEM = "LCD 2004 (I2C: 0x3F)";
const char INTL_DISPLAY_WIFI_INFO[] PROGMEM = "Mostrar informació del WiFi";
const char INTL_DISPLAY_DEVICE_INFO[] PROGMEM = "Mostrar informació del dispositiu";

#define INTL_STATIC_IP_TEXT ""
const char INTL_STATIC_IP[] PROGMEM = "";
const char INTL_STATIC_SUBNET[] PROGMEM = "";
const char INTL_STATIC_GATEWAY[] PROGMEM = "";
const char INTL_STATIC_DNS[] PROGMEM = "";

const char INTL_DEBUG_LEVEL[] PROGMEM = "Debug&nbsp;Level";
const char INTL_MEASUREMENT_INTERVAL[] PROGMEM = "Interval de mesura";
const char INTL_DURATION_ROUTER_MODE[] PROGMEM = "Durada del mode d'encaminador";
const char INTL_MORE_APIS[] PROGMEM = "Altres API";
const char INTL_SEND_TO_OWN_API[] PROGMEM = "Enviar a la pròpia API";
const char INTL_SERVER[] PROGMEM = "Servidor";
const char INTL_PATH[] PROGMEM = "Ruta";
const char INTL_PORT[] PROGMEM = "Port";
const char INTL_USER[] PROGMEM = "Usuari";
const char INTL_PASSWORD[] PROGMEM = "Contrasenya";
const char INTL_MEASUREMENT[] PROGMEM = "Mesura";
const char INTL_SEND_TO[] PROGMEM = "Enviar a {v}";
const char INTL_READ_FROM[] PROGMEM = "Llegir de {v}";
const char INTL_SENSOR_IS_REBOOTING[] PROGMEM = "El sensor s'està reiniciant.";
const char INTL_RESTART_DEVICE[] PROGMEM = "Reiniciar dispositiu";
const char INTL_DELETE_CONFIG[] PROGMEM = "Esborrar configuració actual";
const char INTL_RESTART_SENSOR[] PROGMEM = "Reiniciar sensor";
#define INTL_HOME "Inici"
#define INTL_BACK_TO_HOME "Tornar a l'inici"
const char INTL_CURRENT_DATA[] PROGMEM = "Dades actuals";
const char INTL_DEVICE_STATUS[] PROGMEM = "Estat del dispositiu";
#define INTL_ACTIVE_SENSORS_MAP "Mapa de sensors actius (enllaç extern)"
#define INTL_CONFIGURATION_DELETE "Esborrar configuració"
#define INTL_CONFIGURATION_REALLY_DELETE "Estàs segur que vols esborrar la configuració?"
#define INTL_DELETE "Esborrar"
#define INTL_CANCEL "Cancel·lar"
#define INTL_REALLY_RESTART_SENSOR "Estàs segur que vols reiniciar el sensor?"
#define INTL_RESTART "Reiniciar"
const char INTL_SAVE_AND_RESTART[] PROGMEM = "Desar i reiniciar";
#define INTL_FIRMWARE "Versió del Firmware"
const char INTL_DEBUG_SETTING_TO[] PROGMEM = "Ajustar Debug a ";
#define INTL_NONE "off"
#define INTL_ERROR "Error"
#define INTL_WARNING "Advertència"
#define INTL_MIN_INFO "mín. info"
#define INTL_MED_INFO "mitj. info"
#define INTL_MAX_INFO "màx. info"
#define INTL_CONFIG_DELETED "Configuració esborrada"
#define INTL_CONFIG_CAN_NOT_BE_DELETED "La configuració no es pot esborrar"
#define INTL_CONFIG_NOT_FOUND "Configuració no trobada"
const char INTL_TIME_TO_FIRST_MEASUREMENT[] PROGMEM = "Falten {v} segons per a la primera mesura.";
const char INTL_TIME_SINCE_LAST_MEASUREMENT[] PROGMEM = "segons des de l'última mesura.";
const char INTL_PARTICLES_PER_LITER[] PROGMEM = "Partícules/litre";
const char INTL_PARTICULATE_MATTER[] PROGMEM = "partícules fines";
const char INTL_TEMPERATURE[] PROGMEM = "Temperatura";
const char INTL_HUMIDITY[] PROGMEM = "Humitat";
const char INTL_PRESSURE[] PROGMEM = "Pressió atmosfèrica";
const char INTL_DEW_POINT[] PROGMEM = "Punt de rosada";
const char INTL_CO2_PPM[] PROGMEM = "ppm CO₂";
const char INTL_LEQ_A[] PROGMEM = "LAeq";
const char INTL_LA_MIN[] PROGMEM = "LA min";
const char INTL_LA_MAX[] PROGMEM = "LA max";
const char INTL_LATITUDE[] PROGMEM = "Latitud";
const char INTL_LONGITUDE[] PROGMEM = "Longitud";
const char INTL_ALTITUDE[] PROGMEM = "Altitud";
const char INTL_TIME_UTC[] PROGMEM = "Hora (UTC)";
const char INTL_SIGNAL_STRENGTH[] PROGMEM = "Intensitat del senyal";
const char INTL_SIGNAL_QUALITY[] PROGMEM = "Qualitat del senyal";
#define INTL_NUMBER_OF_MEASUREMENTS "Nombre de mesures"
#define INTL_TIME_SENDING_MS "Temps invertit en enviar"
#define INTL_SENSOR "Sensor"
#define INTL_PARAMETER "Paràmetre"
#define INTL_VALUE "Valor"

#include "./airrohr-logo-common.h"