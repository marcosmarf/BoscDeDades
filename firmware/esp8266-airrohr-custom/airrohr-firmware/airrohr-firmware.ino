...
. Variables and constants in RAM (global, static), used 42592 / 80192 bytes (53%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ DATA     2724     initialized variables
╠══ RODATA   9476     constants       
╚══ BSS      30392    zeroed variables
. Instruction RAM (IRAM_ATTR, ICACHE_RAM_ATTR), used 63621 / 65536 bytes (97%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ ICACHE   32768    reserved space for flash instruction cache
╚══ IRAM     30853    code in IRAM    
. Code in flash (default, ICACHE_FLASH_ATTR), used 643208 / 1048576 bytes (61%)
║   SEGMENT  BYTES    DESCRIPTION
╚══ IROM     643208   code in flash  
...
// increment on change
#define SOFTWARE_VERSION_STR "AIRROHR-DNMS-5.16"
...
unsigned dnms_2nd_threshold = 20;
...
bool with_wlanpwd = WITH_WLANPWD;
...
static void webserver_config_send_body_get(String &page_content) {
  ...
  page_content += FPSTR(TABLE_TAG_OPEN);   
  add_form_checkbox(Config_with_wlanpwd, FPSTR(INTL_WIFI_WITH_PWD));
  page_content += FPSTR(TABLE_TAG_OPEN);  
...
*/
  ...
  server.sendContent(page_content);
  page_content = form_checkbox(Config_send2influx, tmpl(FPSTR(INTL_SEND_TO), F("InfluxDB (V1, V2 only with 1.x compatibility mode)")), false);
  //  page_content += FPSTR(WEB_NBSP_NBSP_BRACE);
...
static void wifiConfig() {
  ...
  if (cfg::with_wlanpwd) {   // is wlanpwd empty then there is no need for Wifi.begin with password
...
*/
      ...
      transmit_result = http.POST(data);

      if (transmit_result >= HTTP_CODE_OK && transmit_result <= 299) {
        debug_outln_info(F("Succeeded - "), s_Host);
        send_success = true;
      } else if (transmit_result >= HTTP_CODE_BAD_REQUEST || transmit_result < 0) {

        Debug.print("error transmit_result: ");
        Debug.println(transmit_result);
        ...
        //      debug_outln_error(String(transmit_result));
        switch (transmit_result) {
          case -1:
            debug_outln("connection failed", DEBUG_ERROR);
            for (ii = 0; ii < 3; ii++) {
              delay(50);
              transmit_result = http.POST(data);
              Debug.print("retransmission transmit_result: ");
              Debug.println(transmit_result);
              if (transmit_result >= HTTP_CODE_OK && transmit_result <= 299) {
                send_success = true;
                break;
          ...
          case -3:
            debug_outln("send payload failed", DEBUG_ERROR);
            for (ii = 0; ii < 3; ii++) {
              delay(50);
              transmit_result = http.POST(data);
              Debug.print("retransmission transmit_result: ");
            ...
            break;
          case -4:
            debug_outln("not connected", DEBUG_ERROR);
            break;
            for (ii = 0; ii < 3; ii++) {
              delay(50);
              transmit_result = http.POST(data);
            ...
            break;
          case -5:
            debug_outln("connection lost", DEBUG_ERROR);
            for (ii = 0; ii < 3; ii++) {
              delay(50);
              transmit_result = http.POST(data);
              Debug.print("retransmission transmit_result: ");
          ...
          case -11:
            debug_outln("read timeout", DEBUG_ERROR);
            for (ii = 0; ii < 3; ii++) {
              delay(50);
              transmit_result = http.POST(data);
              Debug.print("retransmission transmit_result: ");
            ...
            break;
          default: debug_outln_error(String(transmit_result)); break;
        }
    ...
    if (!send_success && transmit_result != 0) {
      loggerConfigs[logger].errors++;
      last_sendData_returncode = transmit_result;
    }
  }
...
static void create_influxdb_string_from_data(String &data_4_influxdb, const String &data, const String &measure_name) {
  ...
  if (!err) {
    ...
    // check if DNMS is configured, if not get time fpr timestamp
    if (!(cfg::dnms_read_active || cfg::dnms_2nd_interval_active)) {
